...
function __bobthefish_prompt_status -d 'the symbols for a non zero exit status, root and background jobs'
  ...
  set last_exit $RETVAL
  if [ $last_exit -ne 0 ]
    set nonzero "$nonzero_exit_glyph$last_exit "
  end
...
function __bobthefish_prompt_user -d 'Display actual user if different from $default_user'
  ...
  echo -n -s (whoami) ' '
  __bobthefish_start_segment $slate_blue $lt_grey
...
function __bobthefish_virtualenv -d 'Display the active virtualenv'
